 ...
 *
 * Two transports over the SAME dispatcher, no SDK, matching this package's
 * zero-runtime-dependency rule:
 *   - stdio   — newline-delimited JSON-RPC 2.0 (startMcpServer). The default,
 *               for Claude Code / Claude Desktop.
 *   - HTTP    — the MCP Streamable HTTP transport (startMcpHttpServer): a
 *               single POST endpoint returning JSON, a GET keep-alive stream,
 *               optional bearer-token auth. This is what a remote connector
 *               (claude.ai custom connector, reached through a tunnel) speaks.
 *
 * Each tool calls the shared application service (lib/app/job-cv.js) in-process
 * — the SAME instance the preview page's routes use, so a tool call and the
 * browser see one state.
 *
    ...
    async run({ app, sid, ui }) {
      ...
      return {
        ...
        active: {
          ...
          baseMasterVersion: doc.baseMasterVersion || 0,
          // The master has moved on since this CV was reconciled with it —
          // jobcv_get what:"master-delta" shows what it gained, to sync down.
          masterSyncAvailable:
            (doc.masterVersion || 0) > 0 &&
            (doc.version || 0) > 0 &&
            (doc.masterVersion || 0) > (doc.baseMasterVersion || 0),
  ...
  },
  {
    ...
    schema: {
      ...
      properties: {
        what: {
          ...
          description:
            'Which document or list to return. "delta" is what THIS CV changed against the master (the fold-back view); "master-delta" is what the master GAINED since this CV was reconciled with it (the sync-down view).',
        },
        version: {
          type: 'integer',
    ...
    async run({ app, sid }, args) {
      switch (args.what) {
        ...
        case 'master-delta':
          return app.getDelta(sid, 'cv', 'incoming')
        case 'profile':
    ...
    schema: {
      ...
      properties: {
        ...
        baseMasterVersion: {
          ...
          minimum: 0,
          description:
            'Set ONLY on a sync save: the master CV version you reconciled this CV against (from jobcv_get what:"master-delta"). Marks the lineage so the next sync starts from there. Leave unset on an ordinary tailoring save.',
        },
      },
    },
    ...
    schema: {
      ...
      properties: {
        ...
        jobUrl: {
          type: 'string',
          description:
            'Tag a job other than the active one — its posting link, from jobcv_context.applications. Omit to tag the active candidacy.',
        },
      },
    },
 ...
 *   { result }               — a reply for a request
 *   { error: {code,message} } — a failure for a request
 *   undefined                 — a notification, or a method with no reply
 * Both transports call this and frame the answer their own way.
 */
async function dispatch(msg, ctx) {
  const { app, sid, ui } = ctx
  ...
  if (method === 'resources/read') {
    ...
    if (uri === 'jobcv://skill') {
      return ok({ contents: [{ uri, mimeType: 'text/plain', text: app.getSkill() }] })
    }
    if (uri === 'jobcv://profile') {
      const p = await app.getProfile(sid)
      return ok({ contents: [{ uri, mimeType: 'text/markdown', text: p.text || '' }] })
    }
  ...
  if (!isNotification) return err(-32601, 'method not found: ' + method)
}

const hasId = (msg) => msg && msg.id !== undefined && msg.id !== null

/* ─────────────────────────── stdio transport ────────────────────────── */

export function startMcpServer(options) {
  const ui = options.ui
  const ctx = { app: ui.app, sid: ui.sessionId, ui }
  ...
  stdin.on('data', (chunk) => {
    ...
    while ((nl = buffer.indexOf('\n')) !== -1) {
      ...
      inFlight++
      Promise.resolve(dispatch(msg, ctx))
        .then((out) => {
          if (out && hasId(msg)) send({ jsonrpc: '2.0', id: msg.id, ...out })
        })
        .catch((error) => {
  ...
  log('ready — preview at ' + ui.url + ' (session ' + ui.sessionId + ')')
}
...
/**
 * The MCP Streamable HTTP transport. One endpoint (default `/mcp`):
 *   POST  — a JSON-RPC message (or a batch); answers with application/json,
 *           or 202 when every message was a notification.
 *   GET   — an SSE stream for server-initiated messages. We have none, so it
 *           just holds open with keep-alives (more compatible than 405).
 *   DELETE— ends the session (a no-op here; the jobcv session is the server's).
 * A bearer token (options.token) gates everything but OPTIONS and /health.
 * Bind to 127.0.0.1 and put a tunnel (cloudflared, ngrok) in front for a
 * public URL; never expose it to a network without a token.
 */
export function startMcpHttpServer(options) {
  ...
  const server = createServer((req, res) => {
    ...
    if (pathname === '/health' || pathname === '/') {
      return json(res, 200, { ok: true, server: SERVER_INFO, endpoint })
    }
    ...
    req.on('end', async () => {
      ...
      for (const msg of msgs) {
        let answer
        try {
          answer = await dispatch(msg, ctx)
        } catch (error) {
          answer = {
            error: {
              code: -32603,
              message: 'internal error: ' + String(error && error.message ? error.message : error),
            },
          }
        }
        if (answer && hasId(msg)) out.push({ jsonrpc: '2.0', id: msg.id, ...answer })
      }
      const extra = { 'mcp-session-id': mcpSessionId }
      if (out.length === 0) {
        res.writeHead(202, { ...cors, ...extra })
        return res.end()
      }
      json(res, 200, batch ? out : out[0], extra)
    })
  })
  ...
  return new Promise((resolve, reject) => {
    ...
    server.listen(port, host, () => {
      ...
      const url = 'http://' + shown + ':' + actualPort + endpoint
      if (host !== '127.0.0.1' && host !== 'localhost' && !token) {
        log(
          'WARNING: HTTP transport bound to ' +
            host +
            ' with no --http-token — anyone who reaches it can drive your CV workspace',
        )
      }
      resolve({
        url,
