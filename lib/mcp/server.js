 ...
 * package's zero-runtime-dependency rule. It exposes the SAME document
 * surface the DSH plugin does, as typed tools, and points the client at a
 * live preview page (served by ./ui-server.js) so the user watches the CV
 * render exactly as they would inside the harness.
 *
...
/**
 * Each tool: a JSON-schema for the client, and a `run(call, args)` that maps
 * onto one or more `/jobcv/*` requests. `call(method, path, {query, body})`
 * injects the session id and returns { status, json }.
 */
const TOOLS = [
  {
    name: 'jobcv_context',
    description:
      'Where the workspace stands: the preview URL to give the user, the active candidacy (company, title, job link, CV version), the current fit score, any pending change proposal, and how many applications are tracked. Call this first.',
    schema: { type: 'object', properties: {} },
    async run(call, _args, ui) {
      const doc = await call('GET', '/jobcv/doc')
      const apps = await call('GET', '/jobcv/applications')
      return {
        previewUrl: ui.url,
        note: 'Open previewUrl in a browser to watch the CV, letter, job post and fit score update live.',
        active: {
          company: doc.json.company || '',
          jobTitle: doc.json.jobTitle || '',
          jobUrl: doc.json.jobUrl || '',
          workspace: doc.json.workspace || '',
          cvVersion: doc.json.version || 0,
          letterVersion: doc.json.letter ? doc.json.letter.version : 0,
          masterVersion: doc.json.masterVersion || 0,
          fitScore: doc.json.fit ? doc.json.fit.score : null,
          status: doc.json.status || 'drafting',
          pendingProposal: doc.json.proposal ? doc.json.proposal.id : null,
        },
        applications: (apps.json.applications || []).map((a) => ({
          company: a.company,
          jobTitle: a.jobTitle,
  ...
  },
  {
    ...
    async run(call, args) {
      const sw = await call('POST', '/jobcv/switch', {
        body: { jobUrl: args.jobUrl, company: args.company, jobTitle: args.jobTitle },
      })
      const ws = await call('POST', '/jobcv/workspace', {
        body: {
  ...
  },
  {
    ...
    async run(call, args) {
      const map = {
        ...
        master: '/jobcv/master',
        fit: '/jobcv/fit',
        workspace: '/jobcv/workspace',
        delta: '/jobcv/delta',
        history: '/jobcv/history',
        applications: '/jobcv/applications',
        candidacies: '/jobcv/candidacies',
        joblist: '/jobcv/joblist',
        skill: '/jobcv/skill',
      }
      const query = {}
      if (args.version != null) query.version = args.version
      if (args.kind) query.kind = args.kind
      const r = await call('GET', map[args.what], { query })
      return r.json
    },
  },
  {
    ...
    async run(call, args) {
      const r = await call('POST', '/jobcv/doc', {
        body: { html: args.html, note: args.note, jobUrl: args.jobUrl },
      })
      return r.json
    },
  },
  {
    ...
    async run(call, args) {
      const r = await call('POST', '/jobcv/letter', {
        body: { html: args.html, note: args.note, jobUrl: args.jobUrl },
      })
      return r.json
    },
  },
  {
    ...
    async run(call, args) {
      const r = await call('POST', '/jobcv/master', { body: { html: args.html, note: args.note } })
      return r.json
    },
  },
  {
    ...
    async run(call, args) {
      const r = await call('POST', '/jobcv/post', {
        body: { text: args.text, source: args.source, html: args.html, jobUrl: args.jobUrl },
      })
      return r.json
    },
  },
  {
    ...
    async run(call, args) {
      const r = await call('POST', '/jobcv/brief', {
        body: { sections: args.sections, meta: args.meta, jobUrl: args.jobUrl },
      })
      return r.json
    },
  },
  {
    ...
    async run(call, args) {
      const r = await call('POST', '/jobcv/fit', {
        body: {
          score: args.score,
  ...
  },
  {
    ...
    async run(call, args) {
      const r = await call('POST', '/jobcv/proposal', {
        body: { summary: args.summary, changes: args.changes, jobUrl: args.jobUrl },
      })
      return {
        ...r.json,
  ...
  },
  {
    ...
    async run(call, args) {
      const r = await call('POST', '/jobcv/switch', {
        body: { jobUrl: args.jobUrl, company: args.company, jobTitle: args.jobTitle },
      })
      return r.json
    },
  },
  {
    ...
    async run(call, args) {
      const r = await call('POST', '/jobcv/status', {
  ...
  },
  {
    ...
    async run(call, args) {
      const r = await call('POST', '/jobcv/restore', {
        body: { version: args.version, kind: args.kind },
      })
      return r.json
    },
  },
  {
    ...
    async run(call, args) {
      const r = await call('POST', '/jobcv/joblist', { body: { path: args.path } })
      return r.json
    },
  },
]
...
export function startMcpServer(options) {
  ...
  /** One `/jobcv/*` request with the session id injected. */
  async function call(method, path, opts) {
    const o = opts || {}
    const u = new URL(path, ui.url)
    u.searchParams.set('session', ui.sessionId)
    if (o.query) {
    ...
    if (o.body !== undefined) {
      ...
      init.body = JSON.stringify({ ...o.body, sessionId: ui.sessionId })
    }
    ...
    if (ct.includes('application/json')) {
      try {
        ...
        json = { error: 'unparseable response', raw: text.slice(0, 500) }
      }
    } else {
      json = { text }
    }
  ...
  async function handle(msg) {
    ...
    if (method === 'tools/call') {
      ...
      try {
        const out = await tool.run(call, (params && params.arguments) || {}, ui)
        return reply(id, {
    ...
    if (method === 'resources/read') {
      ...
      if (uri === 'jobcv://skill') {
        const r = await call('GET', '/jobcv/skill')
        const text = typeof r.json.text === 'string' ? r.json.text : JSON.stringify(r.json)
        return reply(id, { contents: [{ uri, mimeType: 'text/plain', text }] })
      }
      if (uri === 'jobcv://context') {
        const out = await TOOL_BY_NAME.get('jobcv_context').run(call, {}, ui)
        return reply(id, {
  ...
  stdin.on('end', () => {
    ended = true
    // A client that hung up mid-request still gets its reply before the
    // process leaves; a wedged handler cannot hold shutdown forever.
    setTimeout(() => {
      inFlight = 0
      maybeShutDown()
    }, 2000).unref()
