 ...
 * The routes and the MCP tools share ONE app instance.
 */
...
/**
 * @typedef {object} UiServerOptions
 * @property {string} sessionId  the MCP shell's stable session id
 * @property {string} [applicationsRoot]  where candidacy folders go
 * @property {number} [port]  0 (default) lets the OS choose
 * @property {string} [host]  '127.0.0.1' by default
 *
 * @typedef {object} UiServer
 * @property {string} url
 * @property {number} port
 * @property {string} sessionId
 * @property {ReturnType<import('../store/doc-store.js').createDocStore>} store
 * @property {ReturnType<import('../app/job-cv.js').createJobCvApp>} app
 ...
 *
 * @param {UiServerOptions} options
 * @returns {Promise<UiServer>}
 */
export async function startUiServer(options) {
  const opts = /** @type {UiServerOptions} */ (options || {})
  ...
  const port = typeof address === 'object' && address !== null ? address.port : Number(opts.port)
  return {
    url: 'http://' + host + ':' + port + '/',
    port,
    sessionId,
