...
function skillInstructions() {
  ...
  return [
    ...
    '',
    '  SYNCING AN IMPROVED MASTER BACK DOWN INTO A TAILORED CV. The other',
    '  direction: when the master has moved on since a candidacy was tailored,',
    '  its new material may belong in that CV too. Read what the master GAINED',
    '  since this CV last reconciled with it:',
    '',
    '    GET ' + base + '/jobcv/delta?session=<sessionId>&dir=incoming',
    '',
    '  Same mechanical block diff, master@base → master@HEAD, with',
    '  baseMasterVersion and masterVersion (and baseInferred:true when the CV',
    '  predates the marker and the base was guessed from the oldest master',
    '  still kept). empty:"in-sync" means there is nothing to pull in.',
    '  This is NOT a merge you run: decide which of those additions actually',
    '  serve THIS job post — not all will — PROPOSE those through',
    '  /jobcv/proposal, say plainly which you are leaving out and why, and',
    '  apply only what the user accepts. Save with the master version you',
    '  reconciled against, so the next sync starts from there:',
    '',
    '    POST ' + base + '/jobcv/doc',
    '    {"sessionId":"<id>","html":"<full document>","note":"Synced master v<M>",',
    '     "baseMasterVersion":<M>,"jobUrl":"<link>"}',
    '',
    '  A change the user skips here is gone from future incoming deltas; the',
    '  master still holds it and /jobcv/delta (outgoing) still shows it, so',
    '  nothing is lost.',
    '',
