...
import { readRequests, newRequest } from './requests.js'
import {
...
function emptyRecord() {
  return {
    ...
    company: '',
    jobTitle: '',
    // The master version this CV was last reconciled against — seeded to the
    // master HEAD on the first save, carried forward on ordinary saves, moved
    // only by a deliberate sync. `deltaVsMaster(…, 'cv', 'incoming')` diffs
    // this against the current master to show what the master gained since.
    baseMasterVersion: 0,
    history: [],
  }
}
...
export function normalizeRecord(parsed) {
  ...
  return {
    ...
    requests: readRequests(raw.requests),
    company: typeof raw.company === 'string' ? raw.company : '',
    jobTitle: typeof raw.jobTitle === 'string' ? raw.jobTitle : '',
    // A record written before this field existed reads as 0 — "never
    // reconciled" — and the incoming delta infers a base from the oldest
    // master version it still has.
    baseMasterVersion:
      Number.isInteger(raw.baseMasterVersion) && raw.baseMasterVersion >= 0
        ? raw.baseMasterVersion
        : 0,
...
export function createDocStore(rootDir) {
  ...
  // The standing facts about the candidate, beside the master — one person,
  // many applications, one profile every session would otherwise re-derive.
  const profiles = createProfileStore(rootDir)
  ...
  function save(sessionId, doc) {
    return withSessionLock(sessionId, async function () {
      ...
      // The master version this CV is reconciled against. Explicit on a sync
      // save (the agent passes the version it just diffed against); carried
      // forward on an ordinary tailoring save; and seeded to the master HEAD
      // on the FIRST save, because a fresh CV is tailored from the master as
      // it stands right then — so its first "what's new in the master" delta
      // is empty rather than the whole document.
      let baseMasterVersion = record.baseMasterVersion || 0
      if (Number.isInteger(doc.baseMasterVersion) && doc.baseMasterVersion >= 0) {
        baseMasterVersion = doc.baseMasterVersion
      } else if (record.version === 0) {
        try {
          const m = await masters.load()
          if (m.version > 0 && m.html !== '') baseMasterVersion = m.version
        } catch {
          /* no master yet, or unreadable — 0 ("never reconciled") is honest */
        }
      }
      const next = {
        ...
        baseMasterVersion: baseMasterVersion,
        ...
        // Pending requests from the preview survive a save; the agent resolves
        // them explicitly when done.
  ...
  function restore(sessionId, version) {
    return withSessionLock(sessionId, async function () {
      ...
      const next = {
        ...
        company: record.company,
        jobTitle: record.jobTitle,
        // A rollback of the CV body is not a re-reconciliation with the
        // master: the lineage marker stays where it was.
        baseMasterVersion: record.baseMasterVersion || 0,
        // A save answers whatever was pending: leaving the proposal up would
        // ask the user to decide again about text that no longer exists.
        proposal: null,
        fit: record.fit,
  ...
  /**
   * Add a request from the preview to the inbox (newest first, bounded).
   * Shares the write lock; pushes a frame so an open preview and jobcv_context
   * both see it.
   */
  function addRequest(sessionId, input) {
    return withSessionLock(sessionId, async function () {
      const record = await load(sessionId)
      const request = newRequest(input)
      const next = Object.assign({}, record, {
        requests: [request, ...record.requests].slice(0, 20),
      })
      await persist(sessionId, next)
      remember(sessionId, next)
      notify(sessionId)
  ...
  /**
   * Drop requests by id (or all when ids is omitted/empty) — the agent
   * resolving what it has acted on, or the user dismissing one.
   */
  function resolveRequests(sessionId, ids) {
    return withSessionLock(sessionId, async function () {
      const record = await load(sessionId)
      const drop = Array.isArray(ids) && ids.length > 0 ? new Set(ids.map(String)) : null
      const kept = drop === null ? [] : record.requests.filter((r) => !drop.has(r.id))
      const next = Object.assign({}, record, { requests: kept })
      await persist(sessionId, next)
      remember(sessionId, next)
      notify(sessionId)
      return kept
  ...
  async function get(sessionId) {
    ...
    return {
      ...
      letter: record.letter,
      note: record.note,
      // Pending asks from the preview — full objects, they are small and
      // bounded. jobcv_context surfaces them; the agent resolves them.
      requests: record.requests,
      ...
      // The master version this CV was last reconciled against. A HEAD past
      // this means the master has moved on and a sync is available; 0 means
      // the CV predates the marker (also treated as "sync available").
      baseMasterVersion: record.baseMasterVersion || 0,
    }
  }
  ...
  /** The candidate profile — standing facts, not any one application. */
  async function getProfile() {
    return profiles.load()
  }

  /** Save the profile. The sessionId only routes the notification. */
  function saveProfile(sessionId, text) {
    return profiles.save(text).then(function (stored) {
      if (sessionId) notify(sessionId)
      return stored
    })
  }
  ...
  /**
   * The oldest master version still on disk — the current one, or the
   * lowest-numbered entry in its retained history. It is the furthest back
   * an incoming delta can honestly reach when a CV's own base marker has
   * fallen off the far end of that history.
   */
  function oldestMasterVersion(master) {
    const past = master.history.map((e) => e.version).filter((v) => Number.isInteger(v) && v > 0)
    return past.length > 0 ? Math.min(...past) : master.version
  }

  /**
   * A block diff between the master CV and one candidacy document.
   *
   ...
   * entries. Two directions, one computation:
   *
   *   'outgoing' (default) — master@HEAD BEFORE, the tailored doc AFTER.
   *     Additions are what this application gained, removals what tailoring
   *     left out. This is the fold-back view and the preview's "vs master"
   *     panel. Works for the CV and the cover letter.
   *
   *   'incoming' — master@base BEFORE, master@HEAD AFTER, where `base` is the
   *     version this CV was last reconciled against (`record.baseMasterVersion`,
   *     inferred from the oldest retained master when the CV predates the
   *     marker). Additions are what the master has GAINED since — the
   *     candidate for syncing down into this CV. CV only.
   *
   * Both are compact by construction, so reasoning over many applications
   * costs many small deltas rather than many whole CVs.
   */
  async function deltaVsMaster(sessionId, kind, direction) {
    const record = await load(sessionId)
    const master = await masters.load()
    const incoming = direction === 'incoming'
    const base = {
      ok: true,
      kind: incoming ? 'cv' : kind === 'letter' ? 'letter' : 'cv',
      direction: incoming ? 'incoming' : 'outgoing',
      masterVersion: master.version,
      masterUpdatedAt: master.updatedAt,
    }
    const zeros = { added: 0, removed: 0, same: 0, changes: [], truncated: false }
    if (master.version < 1 || master.html === '') {
      return Object.assign(base, zeros, { empty: 'no-master', targetVersion: 0 })
    }

    if (incoming) {
      // The CV whose lineage we are comparing — it has to exist to sync into.
      if (record.version < 1 || record.html === '') {
        return Object.assign(base, zeros, {
          empty: 'no-document',
          targetVersion: 0,
          baseMasterVersion: 0,
        })
      }
      let baseVersion = record.baseMasterVersion || 0
      let baseInferred = false
      if (baseVersion < 1 || baseVersion > master.version) {
        baseVersion = oldestMasterVersion(master)
        baseInferred = true
      }
      let baseHtml =
        baseVersion >= master.version ? master.html : await masters.versionHtml(baseVersion)
      if (baseHtml === null) {
        // The stamped base has aged out of the retained history: reach as far
        // back as the history still allows and say the base was inferred.
        baseVersion = oldestMasterVersion(master)
        baseInferred = true
        baseHtml =
          baseVersion >= master.version ? master.html : await masters.versionHtml(baseVersion)
      ...
      const common = Object.assign(base, {
        ...
        baseMasterVersion: baseVersion,
        baseInferred: baseInferred,
      })
      if (baseVersion >= master.version || baseHtml === null || baseHtml === master.html) {
        return Object.assign(common, zeros, {
          empty: 'in-sync',
          same: htmlBlocks(master.html).length,
        })
      }
      const summary = summarizeDiff(diffBlocks(htmlBlocks(baseHtml), htmlBlocks(master.html)))
    ...
    if (!html || targetVersion < 1) {
      return Object.assign(base, zeros, { empty: 'no-document', targetVersion: targetVersion })
    }
  ...
  async function listCandidacies(sessionId) {
    ...
    rows.sort(function (a, b) {
      return Number(b.active) - Number(a.active) || b.activity - a.activity
    })
    return rows.slice(0, MAX_APPLICATIONS)
  }
