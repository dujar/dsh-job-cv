...
export function createDocStore(rootDir) {
   ...
   */
  async function versionHtml(sessionId, version) {
    const record = await load(sessionId)
  ...
  async function history(sessionId) {
    ...
    const entries =
      record.version > 0
        ? [{ version: record.version, updatedAt: record.updatedAt, note: record.note }]
        : []
  ...
  return { load, save, saveLetter, get, setWorkspace, setProposal, restore, history, versionHtml }
}
