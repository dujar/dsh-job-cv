...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    // ------------------------- document sanitizer -------------------------
    // Everything this plugin renders into a sandboxed srcdoc iframe — the CV,
    // the letter, the job post page, a dock file preview — must arrive
    // without executable content. The frames deliberately never get
    // allow-scripts (agent-authored HTML plus same-origin would be a script
    // capability), and the browser logs a blocked-execution error for every
    // <script> it meets DURING THE INITIAL PARSE — before any onLoad defense
    // can remove it, so injectPageDeck's removal can never silence that.
    // The strip happens here instead, before the string reaches a frame.
    //
    // A DOMParser document never runs scripts, so parsing there is safe. Its
    // serialized output only replaces the input when a script was actually
    // present: anything clean returns on the fast path below untouched.
    function stripScriptTags(html) {
      var raw = String(html === undefined || html === null ? '' : html)
      if (!/<script/i.test(raw)) return raw
      try {
    ...
    function loadPrefs(sessionId) {
        ...
        if (raw !== null) {
          ...
          if (parsed && typeof parsed === 'object') {
            return {
              ...
              // Whether the touch swipe hint has been dismissed before: it
              // shows once per session, not again on every reload.
              swipeHintSeen: parsed.swipeHintSeen === true,
            }
          }
        }
      ...
      return { open: true, chatW: null, swipeHintSeen: false }
    }
    function savePrefs(sessionId, prefs) {
      try {
        var key = prefsKey(sessionId)
        // Merge, not replace: the dock and the pane write different keys
        // (open/chatW vs. swipeHintSeen), and a replace would drop the other's.
        var merged = prefs
        try {
          ...
          if (raw !== null) {
            ...
            if (existing && typeof existing === 'object') {
              merged = Object.assign({}, existing, prefs)
            }
          }
        } catch (e) { /* an unreadable pref is treated as empty */ }
    ...
    /**
     * Is this the same document as the last one?
     *
     * A frame that matches is discarded, so what this compares is
    ...
    function sameDoc(a, b) {
      ...
      // A status tag moves with no save at all: it is the user's report, not
      // a document write. The projection carries the three markers the dock
      // paints; if any of them move the frame must not be dropped.
      if ((a.status || '') !== (b.status || '')) return false
      if ((a.statusUpdatedAt || 0) !== (b.statusUpdatedAt || 0)) return false
      if ((a.appliedAt || 0) !== (b.appliedAt || 0)) return false
      // The master CV moves outside any candidacy save (POST /jobcv/master),
      // yet its markers decide whether the preview shows the "vs master"
      // affordance — so a master save must push a fresh frame through.
      if ((a.masterVersion || 0) !== (b.masterVersion || 0)) return false
      if ((a.masterUpdatedAt || 0) !== (b.masterUpdatedAt || 0)) return false
      return true
    }
    ...
    // characters and every save pushes the whole projection, so /jobcv/doc carries
     ...
     *
     * A save is pushed down the stream the moment it lands, and a fast agent
     * can answer in well under a second: without a floor the loading flashes
     * for a few frames and reads as "nothing happened". The floor keeps it on
     * screen long enough to be seen before the update clears it.
     */
...
var EDIT_STRIP_ATTRS = [
  ...
  // The gap taps' once-only guard: it would come back as a document that
  // believes its listeners are already attached.
  'data-dsh-job-cv-gap-taps',
]
...
// Inline styles cannot express a media query either, and the multi-device
// fixes below are all conditional on the POINTER, not on the pane: the same
// window holds a mouse on the desktop and a finger on the phone. Injected
// once into the host page, like the keyframes above.
var TOUCH_STYLE_ID = 'dsh-job-cv-touch'
function ensureTouchStyles() {
  try {
    ...
    style.textContent =
      // A finger needs a bigger target than a mouse: the desktop paddings
      // give a ~22px button, under even the 24px WCAG 2.5.8 floor. min-height
      // only — no inline style sets it, so this wins without an !important
      // war against React's own style objects.
      '@media (pointer: coarse){' +
      '[data-dsh-job-cv-pane] button,[data-dsh-job-cv-overlay] button,' +
      '[data-dsh-job-cv-dock] button{min-height:34px;' +
      '-webkit-tap-highlight-color:transparent}' +
      // An input under 16px makes iOS Safari zoom the whole page when it is
      // focused — on a layout of fixed-position panes that zoom is hard to
      // see past and harder to undo. 16px is the threshold; desktop, where
      // the inline sizes are right, never matches this query.
      '[data-dsh-job-cv-pane] input,[data-dsh-job-cv-pane] textarea,' +
      '[data-dsh-job-cv-overlay] input,[data-dsh-job-cv-overlay] textarea,' +
      '[data-dsh-job-cv-dock] input{font-size:16px!important}' +
      '}' +
      // A phone in overlay mode: the eyebrow repeats what the dock already
      // said, and on a narrow screen it costs a whole wrap line of a toolbar
      // that is already three rows deep.
      '@media (pointer: coarse) and (max-width: 560px){' +
      '[data-dsh-job-cv-eyebrow]{display:none!important}}'
    ;(document.head || document.body).appendChild(style)
  } catch (e) {
    /* touch sizing degrades to the desktop metrics */
  }
}
...
function CvPane(props) {
  ...
  // ---- vs master: what tailoring changed against the source of truth ----
  // One fetch when opened; the panel then describes that moment and says so
  // if the document moves underneath it. Only offered when a master exists
  // at all — the projection's masterVersion marker is what makes this a
  // cheap check instead of a probe request.
  var deltaOpenState = React.useState(false)
  var deltaOpen = deltaOpenState[0]
  var setDeltaOpen = deltaOpenState[1]
  var deltaState = React.useState(null)
  var delta = deltaState[0]
  var setDelta = deltaState[1]
  var deltaLoadingState = React.useState(false)
  var deltaLoading = deltaLoadingState[0]
  var setDeltaLoading = deltaLoadingState[1]
  var deltaErrorState = React.useState(null)
  var deltaError = deltaErrorState[0]
  var setDeltaError = deltaErrorState[1]
  ...
  function toggleDelta() {
    ...
    fetchDelta(props.sessionId, 'cv')
      ...
      .catch(function (err) {
        setDelta(null)
        setDeltaError(String(err && err.message ? err.message : err))
        setDeltaLoading(false)
      })
  }
  ...
  // The agent may mark a weak CV bullet the same way it marks the job post —
  // <mark class="dsh-gap" data-dsh-gap-id="g3">. Paint the convention into the
  // CV frame too (it was post-only), and let a tap open the "what is missing"
  // callout here as well. Same helpers the PostSurface uses.
  React.useEffect(
    function () {
      if (showingPost) return undefined
      var frame = iframeRef.current
      if (!frame) return undefined
      injectPostGapCss(frame)
      var idoc = null
      try {
      ...
      return attachGapTaps(idoc, coarse)
    },
    [showingPost, loadTick, doc.version],
  )
  ...
  function switchView(next) {
    ...
    // The "vs master" panel describes the CV; like a timeline it closes when
    // the tab that owns it does.
    setDeltaOpen(false)
  ...
  function exportPdf() {
    ...
    if (w) {
      // The fallback window is not sandboxed, so a script here would really
      // run — and run with the shell's origin behind it. The contract gives
      // documents no scripts, so the strip is defense, not decoration.
      w.document.open()
      w.document.write(stripScriptTags(html))
      w.document.close()
      if (fileName !== '') {
        try {
  ...
  return createElement(
    ...
    createElement(
      ...
      // must on a narrow pane. Not selectable: a fast double-tap on a phone
      // otherwise lands a text selection over the controls.
      createElement(
        'div',
        {
          style: {
        ...
        createElement(
          'span',
          {
            // The touch stylesheet hides this on a phone (it repeats the dock
            // and costs a wrap line there); the attribute is its handle.
            'data-dsh-job-cv-eyebrow': '',
            style: {
              fontSize: 11,
              fontWeight: 600,
              letterSpacing: 0.6,
        ...
        !onboarding &&
          ...
          : null,
        // "vs master" — the tailored CV against the source of truth. Only on
        // the CV tab (the letter is not tailoring), and only when a master
        // exists to compare against; the projection carries that marker so
        // the button needs no probe request.
        !onboarding &&
          !editing &&
          !showingPost &&
          !showingLetter &&
          !showingLetterPending &&
          (doc.masterVersion || 0) > 0
          ? createElement(
              'button',
              {
                type: 'button',
                'aria-pressed': deltaOpen,
                onClick: toggleDelta,
                title: 'What this tailored CV changed against your master CV',
                style: deltaOpen
                  ? Object.assign({}, toolbarBtn, {
                      background: pal.dark ? 'rgba(122,184,255,0.22)' : 'rgba(46,111,219,0.16)',
                      borderColor: pal.accent,
    ...
    historyOpen && !showingPost
      ...
      : null,
    // vs master panel — what this application's tailoring moved, computed
    // host-side from normalized text blocks
    deltaOpen && !showingPost && !showingLetter
      ? createElement(MasterDeltaPanel, {
          pal: pal,
          delta: delta,
          ...
          stale:
            delta !== null &&
            Number(delta.targetVersion) > 0 &&
            Number(delta.targetVersion) !== doc.version,
          onClose: function () {
            setDeltaOpen(false)
            setDeltaError(null)
          },
      ...
      : createElement(
          'div',
          {
            ...
            style: {
              ...
              padding: '14px 10px',
              position: 'relative',
              // A scroll past the document's end stays here: on a touch
              // device the default chains into the page behind the pane,
              // which reads as the preview losing its own scroll.
              overscrollBehavior: 'contain',
            },
          },
    ...
    createElement(
      ...
      createElement(
        ...
        (props.coarse ? 'tap' : 'click') +
          ' a version to look at it — restoring is a separate step',
      ),
      createElement('span', { style: { flex: 1 } }),
      createElement('button', { type: 'button', onClick: props.onClose, style: btn }, 'Close'),
    ),
    ...
    picked === null
      ...
      : createElement(
          ...
          createElement('textarea', {
            value: props.draft,
            autoFocus: true,
            // The shortcut hint is for a keyboard; a finger has the Send
            // button below and no ⌘ to hold.
            placeholder: props.coarse
              ? 'What needs to change here?'
              : 'What needs to change here? (⌘/Ctrl+Enter sends)',
            onChange: function (e) {
            ...
            return createElement(
              ...
              createElement(
                'span',
                {
                  ...
                  // A mouse still gets the untruncated text on hover.
                  title: note.text,
                },
...
function useSplitPane(col, enabled, pal, chatW, onChatW) {
  ...
  React.useLayoutEffect(
    function () {
      ...
      // A finger cannot land on 7px: on a coarse pointer the handle is wide
      // enough to grab (it stays transparent, so nothing visible changes),
      // and the drag math below centers on whatever width it is.
      ...
      var dividerW = coarsePointer ? 28 : 7
      ...
      function syncLayout() {
        try {
          ...
          divider.style.left = Math.max(0, rect.left + previewW - dividerW / 2) + 'px'
    ...
    createElement(
      'div',
      {
        ...
        style: {
          ...
          display: 'flex',
          flexDirection: 'column',
          // The overlay IS the page while it is up; a scroll that runs past
          // its edges must not chain into the shell underneath it.
          overscrollBehavior: 'contain',
        },
      },
      props.children,
...
function FilePreview(props) {
  ...
  if (!p) return null
  var pal = props.pal
  // The popover must fit the device it is on: a fixed 340px box clips off a
  // 360px phone, and a fixed height runs past a short landscape viewport.
  var vw = window.innerWidth
  var vh = window.innerHeight
  var W = Math.min(340, vw - 16)
  var H = Math.min(320, vh - 16)
  ...
  if (p.loading) {
    ...
    // The empty sandbox blocks everything already; stripping scripts first
    // keeps the browser from logging a blocked-execution error for each one
    // it meets while parsing (see stripScriptTags).
    body = createElement('iframe', {
      srcDoc: stripScriptTags(p.body),
      sandbox: '',
      title: p.name,
...
function JobDock(props) {
  ...
  // The application tracker: a dialog over everything, listing every
  // application with its status tag. The badge count is fetched once when
  // the dock mounts and refreshed whenever the panel closes — not on every
  // poll tick, because the host-side scan reads every session file.
  var appsOpenState = React.useState(false)
  var appsOpen = appsOpenState[0]
  var setAppsOpen = appsOpenState[1]
  var appsCountState = React.useState(null) // null = not known yet
  var appsCount = appsCountState[0]
  var setAppsCount = appsCountState[1]

  // The jobs list: the markdown pick list once one is loaded, and the panel
  // that switches which posting this session works on. Same badge rhythm as
  // the tracker: fetched on mount, refreshed when the panel closes — never
  // per poll tick.
  var jobsOpenState = React.useState(false)
  var jobsOpen = jobsOpenState[0]
  var setJobsOpen = jobsOpenState[1]
  var jobsCountState = React.useState(null)
  var jobsCount = jobsCountState[0]
  var setJobsCount = jobsCountState[1]

  function refreshJobsBadge() {
    fetchJobList(sessionId)
      .then(function (list) {
        setJobsCount(Array.isArray(list.jobs) ? list.jobs.length : 0)
      })
      .catch(function () {
        /* no list loaded yet — the button still opens the loader */
      })
  }
  ...
  React.useEffect(
    function () {
      ...
      fetchApplications(sessionId)
        .then(function (data) {
          if (alive && data && Array.isArray(data.applications))
            setAppsCount(data.applications.length)
        })
        .catch(function () {
          /* the badge is optional; the panel reports its own errors */
        })
      return function () {
        alive = false
      }
  ...
  function openApps(v) {
    setAppsOpen(v)
    if (!v) {
      // Closing the panel re-reads the count, so a change made inside it (or
      // anywhere else) is what the badge says until the next open.
      fetchApplications(sessionId)
        .then(function (data) {
          if (data && Array.isArray(data.applications)) setAppsCount(data.applications.length)
        })
        .catch(function () {
          /* keep the previous badge */
        })
    }
  }
  ...
  // Where THIS application stands, glanceable without opening anything.
  // Clicking it opens the tracker — the same place its tag is edited.
  var statusPill =
    doc.status && doc.status !== 'drafting'
      ? createElement(
          'button',
          {
            type: 'button',
            ...
            title:
              'Marked ' +
              doc.status +
              (doc.appliedAt > 0 ? ' · applied ' + shortDate(doc.appliedAt) : '') +
              ' — open Applications to change it',
            style: {
              display: 'inline-flex',
              alignItems: 'center',
              gap: 5,
              border: '1px solid ' + statusColor(doc.status, pal.dark),
              borderRadius: 999,
              background: 'transparent',
              color: statusColor(doc.status, pal.dark),
              fontSize: 11,
              fontWeight: 600,
              lineHeight: '16px',
              padding: '1px 9px',
          ...
          createElement('span', {
            style: {
              ...
              height: 6,
              borderRadius: 999,
              background: statusColor(doc.status, pal.dark),
              display: 'inline-block',
            },
          }),
          doc.status + (doc.appliedAt > 0 ? ' · ' + shortDate(doc.appliedAt) : ''),
        )
      : null

  var appsButton = createElement(
    'button',
    {
      type: 'button',
      onClick: function () {
      ...
      title:
        'Job application manager — every application with its status tag, latest CV, cover letter and stored job post',
      style: btn,
    },
    appsCount !== null && appsCount > 0 ? 'Applications · ' + appsCount : 'Applications',
  )

  var jobsButton = createElement(
    'button',
    {
      type: 'button',
      onClick: function () {
      ...
      title:
        'Jobs list — pick which posting this session works on, or load a markdown file of postings',
      style: btn,
    },
    jobsCount !== null && jobsCount > 0 ? 'Jobs · ' + jobsCount : 'Jobs',
  )
  ...
  return createElement(
    ...
    createElement(
      'div',
      {
        // The touch stylesheet scopes its bigger targets and 16px inputs by
        // this attribute; the dock's controls live outside the pane.
        'data-dsh-job-cv-dock': '',
        style: {
          display: 'flex',
          alignItems: 'center',
          gap: 8,
    ...
    appsOpen
      ? createElement(ApplicationsPanel, {
          pal: pal,
          sessionId: sessionId,
          inputActions: props.inputActions,
          draft: draft,
          ...
          // A change inside the panel refreshes the dock badge with it.
          onBadge: function () {
            fetchApplications(sessionId)
              .then(function (data) {
    ...
    jobsOpen
      ? createElement(JobListPanel, {
          ...
          onBadge: refreshJobsBadge,
          // Starting a job from the list hands a message to the agent; the
          // working state engages the same way the start form's does.
          onStarted: workStarted,
        })
      : null,
    props.useInput
