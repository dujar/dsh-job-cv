...
import { upsertCandidacy } from '../store/workspace.js'
import { saveIntakeFile, INTAKE_LIMIT } from '../store/intake.js'
...
function defineJobCvRoutes(deps) {
  ...
  const workspaceRoutes = defineRoutes('candidacy', [
    {
      ...
      method: 'POST',
      docs: [['POST', 'upsert <root>/<company>/<job-id>/ for this candidacy']],
      handler: async function (req, res) {
        let body
        try {
          body = await readJsonBody(req)
        ...
        if (result === null) {
          return sendJson(res, 400, { error: 'company and job id/url yield no usable folder name' })
        }
        await store.setWorkspace(sessionId, result.path, body.jobUrl)
    ...
    },
    {
      ...
      handler: async function (req, res) {
        ...
        let staged
        try {
          staged = await saveIntakeFile(deps.intakeRoot, sessionId, body.filename, body.dataBase64)
        } catch (error) {
  ...
  return [docRoutes, workspaceRoutes, skillRoutes]
}
