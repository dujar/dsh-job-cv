...
export function createJobCvApp(deps) {
  ...
  const getDelta = (raw, kind, direction) =>
    store.deltaVsMaster(
      sid(raw),
      kind === 'letter' ? 'letter' : 'cv',
  ...
  async function getProfile(raw) {
    ...
    return { ok: true, text: profile.text, updatedAt: profile.updatedAt }
  }

  async function getRequests(raw) {
    const doc = await store.get(sid(raw))
    return { ok: true, requests: doc.requests || [] }
  }
  ...
  async function saveCv(raw, body) {
    ...
    const version = await store.save(session, {
      ...
      jobUrl: body.jobUrl,
      note: body.note,
      // Present only on a sync save: the master version the agent reconciled
      // this CV against. The store carries the marker forward otherwise.
      baseMasterVersion:
        Number.isInteger(body.baseMasterVersion) && body.baseMasterVersion >= 0
          ? body.baseMasterVersion
          : undefined,
    })
  ...
  /** The preview raises a structured ask; jobcv_context surfaces it. */
  async function addRequest(raw, body) {
    const request = await store.addRequest(sid(raw), {
      kind: body.kind,
  ...
  /** The agent resolves what it has acted on (ids omitted → all). */
  async function resolveRequests(raw, body) {
    const kept = await store.resolveRequests(sid(raw), Array.isArray(body.ids) ? body.ids : null)
    return { ok: true, pending: kept.length }
  }

  async function saveProfile(raw, body) {
    const session = sid(raw)
    if (typeof body.text !== 'string') throw new BadRequest('text must be a string')
    const stored = await store.saveProfile(session, body.text)
    return { ok: true, chars: stored.length }
  }
  ...
  async function setStatus(raw, body) {
    ...
    try {
      // A jobUrl routes the tag into that candidacy's own record — so the
      // Jobs list can retag any row without first making it the active job.
      const application = await store.setApplication(session, {
        status,
        note: body.note,
